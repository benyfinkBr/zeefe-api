...
try {
  ...
  $stmt = $pdo->prepare('SELECT id, brand, last4, exp_month, exp_year, holder_name, card_nickname, status, stripe_payment_method_id, pagarme_card_id, created_at, updated_at FROM customer_cards WHERE client_id = :client ORDER BY updated_at DESC, id DESC');
  ...
  $cards = array_map(function (array $row) {
    ...
    return [
      ...
      'nickname' => $row['card_nickname'] ?? null,
      'status' => $row['status'],
      'provider' => $provider,
