...
function reservation_load(PDO $pdo, int $reservationId): ?array {
  $stmt = $pdo->prepare('SELECT r.*, rooms.name AS room_name, rooms.street AS room_street, rooms.complement AS room_complement, rooms.city AS room_city, rooms.state AS room_state, rooms.daily_rate AS room_daily_rate, rooms.advertiser_id AS advertiser_id, a.fee_pct AS advertiser_fee_pct, a.fee_pct_room AS advertiser_fee_pct_room, clients.name AS client_name, clients.email AS client_email, clients.login AS client_login FROM reservations r JOIN rooms ON rooms.id = r.room_id JOIN clients ON clients.id = r.client_id LEFT JOIN advertisers a ON a.id = rooms.advertiser_id WHERE r.id = ? LIMIT 1');
  ...
  if (empty($row['client_email']) && !empty($row['client_login'])) {
    $row['client_email'] = $row['client_login'];
  }
