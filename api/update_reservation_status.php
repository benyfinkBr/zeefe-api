...
try {
  ...
  if ($action === 'confirm') {
    // Confirma sem acionar cobrança automática (pagamentos desativados)
    $stmtUpd = $pdo->prepare('UPDATE reservations SET status = ?, payment_status = "pendente", hold_expires_at = NULL, updated_at = NOW() WHERE id = ?');
...
function enviarEmailStatusReserva(PDO $pdo, int $reservationId, string $action, string $status): void {
  ...
  switch ($action) {
    case 'confirm':
      $detalhes['bloco_informacoes'] = '<p style="margin:0 0 10px;font-size:15px;line-height:1.6;">Sua reserva foi confirmada. Nossa equipe entrará em contato para combinar o pagamento.</p>';
